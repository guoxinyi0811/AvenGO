...
- `avengo-mark.png`：当前品牌标记、favicon、Apple touch icon 与 manifest 安装图标，来源为用户提供的 Minimal Zine 拼贴 PNG，未重画。
...
- 当前已采用 Minimal Zine Coach Card 视觉层：暖象牙纸底、炭墨、档案标签、复印线条和轻扫描颗粒。
- 核心 token 位于 `:root`：`--paper` / `--card` / `--ink` / `--soft` / `--line*` / `--accent` / `--level-*`。洋红 `--accent:#EF1970` 只用于细线、完成框、关键选择边缘与周期纸条窄标记。
- 标题使用系统衬线栈，标签与数字使用系统等宽栈，正文使用系统中文无衬线栈；没有外部字体。
- 动作清单的勾选与展开已改为真实 button，44px 触控目标、`aria-pressed` / `aria-expanded` 和 `focus-visible` 均已补齐。
- 六段动作说明按固定标签分行，“做对的感觉”仅用一条细洋红边作为扫读锚点；常见错误不用警示色。
- 数字输入使用等宽大字号；过去日实际值在窄屏采用两列自适应网格。
- 375px 与桌面横版均已截图实测无横向溢出。纸张颗粒为轻量内联 SVG，不依赖网络。
...
- 洋红 `#EF1970` 只可作为少量品牌锚点和关键选中 / 完成信号，不能铺满大面积背景或替代三档中性色阶。
